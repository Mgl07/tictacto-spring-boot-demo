...
public class TicTacToeInverted extends Game {
    ...
    public TicTacToeInverted(@Value("${board.size}") int size) {
        super(size);
    ...
    public boolean isGameOver() {
        if (this.isLoser()) {
            System.out.println("Player " + String.valueOf(this.currentPlayer.getSymbol()) + " loses!");
            return true;
    ...
    private boolean isLoser() {
        ...
        if (player != Symbol.EMPTY.getSymbol() && allEqual(player, this.getDiagonal(true))) {
            ...
            return player != Symbol.EMPTY.getSymbol() && allEqual(player, this.getDiagonal(false));
        }
    }
